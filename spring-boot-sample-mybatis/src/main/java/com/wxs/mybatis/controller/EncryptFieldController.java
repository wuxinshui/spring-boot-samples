...
public class EncryptFieldController extends BaseController {
    ...
    public Map testQueryId() {
        User result = userMapper.selectByPrimaryKey(1);
        return Map.of("code", 200, "data", result);
    }

    @RequestMapping(value = "/name", method = RequestMethod.POST)
    public Map testQueryUsername() {
        User result = userMapper.selectByName("addTest");
        return Map.of("code", 200, "data", result);
    }

    @RequestMapping(value = "/password-by-name", method = RequestMethod.POST)
    public Map testQueryPasswordByName() {
        String result = userMapper.selectPasswordByName("addTest");
        return Map.of("code", 200, "data", result);
    }
